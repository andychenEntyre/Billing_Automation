...
pd_df = pd.read_csv("/Users/Andy.Chen/Billing_Automation/9mar26_missing_feb_eligibility_check/Feb_eligibility_batch_log.csv", dtype = str)
day = 1
for _, row in pd_df.iterrows():
    ...
    for i, eligibility_data in enumerate(all_items, start=1):
        ...
        # --- Managed Care Coordinator (CCA/SCO) entity name ---
        managed_care_coordinator = next(
            (item for item in eligibility_data.get("benefitsInformation", [])
            if item.get("name") == "Managed Care Coordinator"),
            None
        )

        entity_name = None
        if managed_care_coordinator:
            entity_name = managed_care_coordinator.get("benefitsRelatedEntity", {}).get("entityName")
        ...
        if df.empty:
            ...
            empty_row = pd.DataFrame([{
                    ...
                    "submitterTransactionIdentifier": eligibility_data.get("submitterTransactionIdentifier", ""),
                    "batchId": eligibility_data.get("batchId", ""),
                    "eligibilitySearchId": eligibility_data.get("eligibilitySearchId", ""),
                    "gender": subscriber.get("gender", ""),
                    "planStatus_0_status": eligibility_data.get("planStatus", [{}])[0].get("status", ""),
                    "mco_entityName": entity_name or "",
                    "run_date": run_date,
                    "run_timestamp": run_timestamp,
                    "status": "No benefitsInformation returned",
                    "errors": json.dumps(eligibility_data.get("errors", [])),
                }]).fillna("").astype(str)
        ...
        df["mco_entityName"] = entity_name or ""
        ...
        if "code" in df.columns and "serviceTypeCodes" in df.columns:
            ...
            after_count = len(df)
            if after_count == 0:
                print(f"🚫 Filtered patient: {full_name}")
                print(f"   memberId: {subscriber.get('memberId','')}")
                print(f"   batchId: {eligibility_data.get('batchId','')}")
                print(f"   eligibilitySearchId: {eligibility_data.get('eligibilitySearchId','')}")
                print(f"   original benefit rows: {before_count}")
                print("------------------------------------------------")
                ...
                empty_row = pd.DataFrame([{
                    ...
                    "run_date": run_date,
                    "run_timestamp": run_timestamp,
                    "status": "Filtered out by service code rules",
                    "errors": json.dumps(eligibility_data.get("errors", [])),
                }]).fillna("").astype(str)
                new_rows_df = pd.concat([new_rows_df, empty_row], ignore_index=True)
                continue
        else:
            print(f"⚠️ Missing expected columns for patient: {full_name}")
    ...
    out_path = "/Users/Andy.Chen/Billing_Automation/9mar26_missing_feb_eligibility_check/Feb_eligibility_results.csv"
